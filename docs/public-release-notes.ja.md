...
この repository は GitHub で public にする前提です。そのため、dataset 由来の media file や学習済み model weight は含めていません。

> [!CAUTION]
> これは interview submission 向けの保守的なコンプライアンスメモです。法律上の助言ではありませんが、dataset 由来 artifact を再配布しない判断を明文化しています。
